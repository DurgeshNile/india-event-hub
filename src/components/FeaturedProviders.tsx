...
const ProviderCard = ({ provider }: { provider: any }) => {
  return (
    <Link to={`/provider/${provider.id}`}>
      <Card className="overflow-hidden transition-all duration-300 hover:shadow-lg h-full bg-gray-800 border-gray-700">
        ...
        <CardContent className="p-4">
          <div className="flex items-start justify-between">
            <div>
              <h3 className="font-bold text-lg mb-1 flex items-center text-white">
                {provider.name}
                {provider.verified && (
                  <CheckCircle size={16} className="ml-2 text-india-blue" />
                )}
              </h3>
              <p className="text-gray-300 text-sm mb-2">{provider.category}</p>
            </div>
            <div className="flex items-center bg-india-cream px-2 py-1 rounded text-sm">
              ...
              <span className="text-xs text-gray-600 ml-1">({provider.reviewCount})</span>
            </div>
          </div>
...
const FeaturedProviders = () => {
  // Filter featured providers and limit to 18
  const featured = serviceProviders.filter(provider => provider.featured).slice(0, 18);
  
  return (
    <section className="py-16 bg-gray-900">
      <div className="container mx-auto px-4">
        <div className="text-center max-w-3xl mx-auto mb-12">
          <h2 className="text-3xl font-bold mb-4 text-white">Featured Service Providers</h2>
